...
class SaleController extends Controller
{
    ...
    public function searchpname(Request $request)
    {
        ...
        $products = Product::with(['brand', 'bottleMappings.bottleProduct'])
    ...
    private function _processApproval($saleReturn)
    {
        ...
        foreach ($items as $item) {
            ...
            $stock = \App\Models\WarehouseStock::updateOrCreate(
                ['product_id' => $productId, 'warehouse_id' => $warehouseId, 'location' => $item['location'] ?? null],
                ['total_pieces' => \DB::raw("total_pieces + $qty")]
            );
            
            // Re-calculate quantity (boxes) based on new total_pieces
    ...
    private function processSale(Request $request, Sale $sale)
    {
        ...
        return DB::transaction(function () use ($request, $sale, $status) {
            ...
            foreach ($productIds as $index => $rawId) {
                ...
                // Handle pipe-separated ID (product_id|warehouse_id|location)
                $parts = explode('|', $rawId);
                $pid = (int) ($parts[0] ?? 0);
                // WH/Loc from ID can be used as fallbacks if row inputs are missing
                // Though in Sale, shared_logic should have populated hidden inputs already.
                ...
                $saleItem->location = $locations[$index] ?? null;
                ...
                // --- AUTO BOTTLE BUNDLING LOGIC ---
                $includeBottle = ($request->include_bottle[$index] ?? 0) == 1;
                $bottlePid = (int) ($request->bottle_product_id[$index] ?? 0);

                if ($includeBottle && $bottlePid > 0) {
                    $bottle = Product::find($bottlePid);
                    if ($bottle) {
                        $bottleItem = new SaleItem;
                        $bottleItem->sale_id = $sale->id;
                        $bottleItem->product_id = $bottlePid;
                        $bottleItem->warehouse_id = $warehouses[$index] ?? 1;
                        $bottleItem->location = $locations[$index] ?? null;
                        $bottleItem->product_name = $bottle->item_name . " (Free Bottle)";
                        
                        // Quantity matches the Attar qty? 
                        // User: "6ml ki bottle auto os mai add hojay ... aik bottle"
                        // Usually 1 bottle per Attar item, but if they sell multiple pieces of Attar, maybe multiple bottles?
                        // "agar prodcut mainy likhi attar hugo boss 6ml tou 6ml ki bottle auto os mai add hojay"
                        // If qty is 1 (one 6ml attar), then 1 bottle.
                        // If qty is 2 (two 6ml attars), then 2 bottles.
                        // For Attar, totalPieces is the ML? No, for Attar, usually they sell by pieces (bottles of attar).
                        // Wait, if it's "by_pieces", totalPieces is the number of attars sold.
                        // So bottle qty = totalPieces.
                        $bottleItem->qty = 1 / ($bottle->pieces_per_box ?: 1); 
                        $bottleItem->total_pieces = 1;
                        $bottleItem->loose_pieces = 0;
                        
                        $bottleItem->price = 0; // FREE
                        $bottleItem->discount_percent = 0;
                        $bottleItem->discount_amount = 0;
                        $bottleItem->total = 0;
                        
                        $bottleItem->brand_id = $bottle->brand_id;
                        $bottleItem->unit_id = $bottle->unit_id;
                        $bottleItem->size_mode = $bottle->size_mode;
                        $bottleItem->save();
                    }
                }
    ...
    private function handleStockImpact(Sale $sale, $type = 'out')
    {
        ...
        foreach ($sale->items as $item) {
            ...
            if ($type === 'out') {
                // Use updateOrCreate to handle missing stock records (negative sales)
    ...
    public function saleBarcodes($id)
    {
        $sale = Sale::with(['items.product'])->findOrFail($id);
        return view('admin_panel.sale.barcodes', compact('sale'));
    }
}
