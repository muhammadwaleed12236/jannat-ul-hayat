...
class PurchaseController extends Controller
{
    ...
    private function upsertStocks(int $productId, float $qtyPiecesDelta, int $branchId, int $warehouseId, ?string $location = null): void
    {
        ...
        if (! $hasGatepass) {
            ...
            foreach ($purchase->items as $item) {
                ...
                $movRows[] = [
                    ...
                    'ref_type' => 'PURCHASE',
                    'ref_id' => $purchase->id,
                    'note' => 'Purchase Confirmed' . ($item->location ? ' (Loc: '.$item->location.')' : ''),
                    'created_at' => now(),
                    'updated_at' => now(),
                ];
                // stocks
                $this->upsertStocks($item->product_id, +$item->qty, $branchId, $itemWarehouseId, $item->location);
            }
    ...
    public function store(Request $request, $gatepassId = null)
    {
        ...
        try {
            $validated = $request->validate([
                ...
                'warehouse_id' => 'nullable|exists:warehouses,id', // Header warehouse is now optional default
                ...
                'item_warehouse_id' => 'nullable|array',
                'item_warehouse_id.*' => 'nullable|exists:warehouses,id',
                'item_location' => 'nullable|array',
                'item_location.*' => 'nullable|string',
            ]);
        ...
        $purchase = DB::transaction(function () use ($validated, $request, $gatepass) {
            ...
            foreach ($pids as $i => $rawId) {
                // Handle pipe-separated ID (product_id|warehouse_id|location)
                $parts = explode('|', $rawId);
                $pid = (int) ($parts[0] ?? 0);
                $idWarehouse = isset($parts[1]) && $parts[1] !== '' ? (int) $parts[1] : null;
                $idLocation = isset($parts[2]) && $parts[2] !== '' ? $parts[2] : null;
                ...
                PurchaseItem::create([
                    ...
                    'product_id' => $pid,
                    'warehouse_id' => $request->item_warehouse_id[$i] ?? $warehouseId,
                    'location' => $request->item_location[$i] ?? null,
                    'unit' => $unit,
                    'price' => $price,
