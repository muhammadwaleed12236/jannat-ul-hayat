...
class ProductController extends Controller
{
    ...
    public function ajaxSearch(Request $request)
    {
        ...
        $query = Product::query()
            ->select('id', 'item_name', 'item_code', 'barcode_path', 'size_mode', 'height', 'width', 'pieces_per_box', 'purchase_price_per_m2', 'purchase_price_per_piece', 'pieces_per_m2', 'purchase_discount_percent', 'sale_discount_percent', 'sub_category_id')
            ->with(['warehouseStocks.warehouse', 'bottleMappings.bottleProduct']) 
        ...
        foreach ($products as $p) {
            $stocks = $p->warehouseStocks;
            // No longer filtering by stock here as user wants to see all racks and sell in minus
            
            $hasStock = $stocks->count() > 0;
            if ($hasStock) {
                foreach ($stocks as $ws) {
                    $stockPieces = (float) $ws->total_pieces;
                    ...
                    $results[] = [
                        'id' => $p->id . '|' . $ws->warehouse_id . '|' . ($ws->location ?: ''), // UNIQUE ID for Select2
                        'product_id' => $p->id,
                        'text' => $p->item_name . " (" . ($ws->warehouse->warehouse_name ?? 'WH') . " - " . ($ws->location ?: 'No Rack') . ")",
                        'sku' => $p->item_code ?? '',
                        'stock' => $stockDisplay,
                        'rack' => $ws->location ?: 'No Rack',
                        'warehouse_name' => $ws->warehouse->warehouse_name ?? 'WH',
                        ...
                        'bottle_mappings' => $p->bottleMappings,
                    ];
                }
            } else {
                // Fallback for products with NO stock entries (New products)
                $results[] = [
                    'id' => $p->id . '||', 
                    'product_id' => $p->id,
                    'text' => $p->item_name . " (New/No Stock)",
                    'sku' => $p->item_code ?? '',
                    'stock' => 0,
                    'rack' => 'No Rack',
                    'warehouse_name' => 'N/A',
    ...
    public function findByBarcode(Request $request)
    {
        ...
        $product = Product::with(['warehouseStocks.warehouse'])
            ->where('item_code', $code)
            ->orWhere('barcode_path', $code)
            ->first();
        ...
        if ($product) {
            ...
            $data = [
                'id' => $product->id . '|' . ($ws ? $ws->warehouse_id : 1) . '|' . ($ws ? $ws->location : 'New'),
                'name' => $product->item_name,
                'item_code' => $product->item_code,
                'stock' => $ws ? (float)$ws->total_pieces : 0,
                'price' => (float)($product->sale_price_per_piece ?: $product->sale_price_per_box),
                'pieces_per_box' => $product->pieces_per_box,
                'size_mode' => $product->size_mode,
                'height' => $product->height,
                'width' => $product->width,
                'warehouse_id' => $ws ? $ws->warehouse_id : 1,
                'location' => $ws ? $ws->location : 'New',
            ];
    ...
    public function searchProducts(Request $request)
    {
        ...
        $products = Product::with(['category_relation', 'sub_category_relation', 'brand', 'warehouseStocks' => function($q) {
        ...
        return response()->json($products->map(function ($p, $key) {
            ...
            // Get unique racks/locations
            $racks = $p->warehouseStocks->pluck('location')->filter()->unique()->implode(', ');
    ...
    // ===== Barcode (API & Print View) =====
    public function generateBarcode(Request $request, $id = null)
    {
        // 1. Check for ID in URL or query string
        $productId = $id ?: $request->query(); 
        // If query() is an array, we might need a specific key, but the user visited ?7 which often results in $request->all() having ['7' => '']
        if (!$id) {
        ...
        // Final fallback if not found and not ajax
        return "Product not found or invalid request.";
    }
