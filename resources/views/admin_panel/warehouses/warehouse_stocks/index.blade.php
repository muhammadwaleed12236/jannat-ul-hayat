    ...
    <div class="container-fluid">
        <div class="card premium-card">
            <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                <h5 class="mb-0 fw-bold text-dark"><i class="fas fa-boxes me-2 text-success"></i> Add Stock / Inventory</h5>
                    ...
                    <table class="table table-hover align-middle" id="stockTable">
                        <thead class="bg-light text-muted">
                            <tr>
                                ...
                                <th class="text-uppercase small fw-bold">Wh / Shop</th>
                                <th class="text-uppercase small fw-bold">Location</th>
                                <th class="text-uppercase small fw-bold">Product</th>
                                <th class="text-uppercase small fw-bold text-center">Boxes</th>
                                <th class="text-uppercase small fw-bold text-center">Pieces</th>
                                <th class="text-uppercase small fw-bold text-end">X</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($stocks as $stock)
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge bg-light text-dark border">{{ $stock->warehouse->warehouse_name ?? 'N/A' }}</span>
                                    </td>
                                    <td>
                                        @if($stock->location)
                                            <span class="badge bg-warning-subtle text-warning border border-warning-subtle">{{ $stock->location }}</span>
                                        @else
                ...
                <div class="modal-body">
                    <form id="stockForm">
                        ...
                        <div class="row g-3">
                            <!-- Warehouse (Col 1) -->
                            <div class="col-md-4">
                                <label class="info-label">Warehouse / Shop</label>
                            ...
                            <!-- Location (New) -->
                            <div class="col-md-4">
                                <label class="info-label">Specific Location (Rack/Bin)</label>
                                <input type="text" name="location" id="location" class="form-control" placeholder="e.g. Rack-A1">
                            </div>
...
@section('js')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#warehouse_id, #location').on('change', function() {
                // If product is already selected, fetch stock overlap
                if (selectedProduct && $('#warehouse_id').val()) {
                    fetchCurrentStock($('#warehouse_id').val(), selectedProduct.id, $('#location').val());
                }
            });
        ...
        function handleProductSelect(data) {
            ...
            */
            ...
            if (whId) {
                fetchCurrentStock(whId, data.id, loc);
            } else {
        ...
        function fetchCurrentStock(warehouseId, productId, location = '') {
            if (!isEditMode) {
